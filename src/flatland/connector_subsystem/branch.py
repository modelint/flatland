...
class Branch:
    ...
    def render(self):
        ...
        # Draw the axis
        asset = f"{self.Connector.Connector_type_name} connector"
        LineSegment.add(layer=layer, asset=asset, from_here=self.Shoot.from_position, to_there=self.Shoot.to_position)
        ...
        for s in self.Hanging_stems:
            ...
            LineSegment.add(layer=layer, asset=asset, from_here=s.Root_end, to_there=Position(x, y))
